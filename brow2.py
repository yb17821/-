...
while True:
    ...
    if go == '1':
        ...
        for q in q_list:
            ...
            if not ans_lt:
                print('还没做过这道题，请先提交。')
                # answers_areas = q.find_elements_by_xpath('.//div[@class="answerArea"]')
                # print('answers_areas',answers_areas)
                # for answers_area in answers_areas:
                #     lis = answers_area.find_elements_by_xpath('.//li')
                #     print('lis',lis)
                #     lis[0].click()
            else:
